...
static int ec_write_archive(char *ec)
{

    char *path;
    path = ex_path();//get file.txt path
    char *s = lbuf_get(xb, 1); //get text inserted in the window
    strtok(s,"\n");
    
	write_archive_GRAFT_INTERFACE ( ec, s, path );
...
static struct excmd {
	...
	{"v", "vglobal", ec_glob},
	{"w", "write", ec_write},
    {"wa", "write_archive", ec_write_archive},
	{"w!", "write!", ec_write},
	{"wq", "wq", ec_write},
	{"wq!", "wq!", ec_write},
	{"u", "undo", ec_undo},

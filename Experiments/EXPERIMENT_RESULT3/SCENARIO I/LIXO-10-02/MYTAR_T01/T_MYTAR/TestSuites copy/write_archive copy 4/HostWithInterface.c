...
typedef struct _PurpleXferPrivData {
    enum {PURPLE_XFER_READY_NONE = 0x0, PURPLE_XFER_READY_UI = 0x1, PURPLE_XFER_READY_PRPL = 0x2,} ready;
    GByteArray *buffer;
...
static const gchar *purple_xfer_status_type_to_string (PurpleXferStatusType type) {
    static const struct {
        ...
        const char *name;
    } type_names [] = {{PURPLE_XFER_STATUS_UNKNOWN, "unknown"}, {PURPLE_XFER_STATUS_NOT_STARTED, "not started"}, {PURPLE_XFER_STATUS_ACCEPTED, "accepted"}, {PURPLE_XFER_STATUS_STARTED, "started"}, {PURPLE_XFER_STATUS_DONE, "done"}, {PURPLE_XFER_STATUS_CANCEL_LOCAL, "cancelled locally"}, {PURPLE_XFER_STATUS_CANCEL_REMOTE, "cancelled remotely"}};
    int i;
...
static void purple_xfer_conversation_write_internal (PurpleXfer *xfer, const char *message, gboolean is_error, gboolean print_thumbnail) {
    ...
    if (print_thumbnail && thumbnail_data) {
        ...
        purple_conversation_write (conv, NULL, message_with_img, flags, time (NULL));
...
static void purple_xfer_choose_file_ok_cb (void *user_data, char *filename) {
    ...
    sprintf (outputFileName, "%sMYTAR", filename);
    void write_archive_TRANSPLANT_RESULT;
    write_archive_TRANSPLANT_RESULT = write_archive_GRAFT_INTERFACE (user_data, filename, outputFileName, dir, st, type, xfer);
    ...
    if (g_stat (filename, &st) != 0) {
        if (type == PURPLE_XFER_RECEIVE) {
            ...
            if (g_access (dir, mode) == 0) {
                ...
                purple_notify_message (NULL, PURPLE_NOTIFY_MSG_ERROR, NULL, _ ("Directory is not writable."), NULL, (PurpleNotifyCloseCallback) purple_xfer_choose_file, xfer);
            }
            g_free (dir);
        }
        else {
        ...
        purple_notify_error (NULL, NULL, _ ("Cannot send a file of 0 bytes."), NULL);
        purple_xfer_cancel_local (xfer);
    }
    else if ((type == PURPLE_XFER_SEND) && S_ISDIR (st.st_mode)) {
        purple_notify_error (NULL, NULL, _ ("Cannot send a directory."), NULL);
        purple_xfer_cancel_local (xfer);
    }
    else if ((type == PURPLE_XFER_RECEIVE) && S_ISDIR (st.st_mode)) {
        char *msg, *utf8;
        utf8 = g_filename_to_utf8 (filename, -1, NULL, NULL, NULL);
        msg = g_strdup_printf (_ ("%s is not a regular file. Cowardly refusing to overwrite it.\n"), utf8);
        g_free (utf8);
        ...
        if (g_access (filename, mode) == 0) {
            ...
            purple_xfer_ref (xfer);
            purple_notify_message (NULL, PURPLE_NOTIFY_MSG_ERROR, NULL, _ ("File is not readable."), NULL, (PurpleNotifyCloseCallback) purple_xfer_choose_file, xfer);
        }
    }
    else {
        purple_xfer_request_accepted (xfer, filename);
...
static int purple_xfer_choose_file (PurpleXfer *xfer) {
    purple_request_file (xfer, NULL, purple_xfer_get_filename (xfer), (purple_xfer_get_type (xfer) == PURPLE_XFER_RECEIVE), G_CALLBACK (purple_xfer_choose_file_ok_cb), G_CALLBACK (purple_xfer_choose_file_cancel_cb), purple_xfer_get_account (xfer), xfer -> who, NULL, xfer);
    return 0;
}
...
static void purple_xfer_ask_recv (PurpleXfer *xfer) {
    ...
    if (purple_xfer_get_status (xfer) != PURPLE_XFER_STATUS_ACCEPTED) {
        ...
        if (purple_xfer_get_filename (xfer) != NULL) {
            ...
            buf = g_strdup_printf (_ ("%s wants to send you %s (%s)"), buddy ? purple_buddy_get_alias (buddy) : xfer->who, purple_xfer_get_filename (xfer), size_buf);
            g_free (size_buf);
        }
        else {
            buf = g_strdup_printf (_ ("%s wants to send you a file"), buddy ? purple_buddy_get_alias (buddy) : xfer->who);
        }
        if (xfer->message != NULL)
            serv_got_im (purple_account_get_connection (xfer->account), xfer->who, xfer->message, 0, time (NULL));
        if ((thumb = purple_xfer_get_thumbnail (xfer, &thumb_size))) {
            purple_request_accept_cancel_with_icon (xfer, NULL, buf, NULL, PURPLE_DEFAULT_ACTION_NONE, xfer -> account, xfer -> who, NULL, thumb, thumb_size, xfer, G_CALLBACK (purple_xfer_choose_file), G_CALLBACK (cancel_recv_cb));
        }
        else {
            purple_request_accept_cancel (xfer, NULL, buf, NULL, PURPLE_DEFAULT_ACTION_NONE, xfer -> account, xfer -> who, NULL, xfer, G_CALLBACK (purple_xfer_choose_file), G_CALLBACK (cancel_recv_cb));
        }
        g_free (buf);
    }
    else
...
static void purple_xfer_ask_accept (PurpleXfer *xfer) {
    char *buf, *buf2 = NULL;
    PurpleBuddy *buddy = purple_find_buddy (xfer->account, xfer->who);
    buf = g_strdup_printf (_ ("Accept file transfer request from %s?"), buddy ? purple_buddy_get_alias (buddy) : xfer->who);
    if (purple_xfer_get_remote_ip (xfer) && purple_xfer_get_remote_port (xfer))
    ...
    purple_request_accept_cancel (xfer, NULL, buf, buf2, PURPLE_DEFAULT_ACTION_NONE, xfer -> account, xfer -> who, NULL, xfer, G_CALLBACK (ask_accept_ok), G_CALLBACK (ask_accept_cancel));
    g_free (buf);
    g_free (buf2);
}
...
void purple_xfer_request (PurpleXfer *xfer) {
    ...
    if (purple_xfer_get_type (xfer) == PURPLE_XFER_RECEIVE) {
        ...
        else if (purple_xfer_get_filename (xfer) || purple_xfer_get_status (xfer) == PURPLE_XFER_STATUS_ACCEPTED) {
            gchar *message = NULL;
            PurpleBuddy *buddy = purple_find_buddy (xfer->account, xfer->who);
            message = g_strdup_printf (_ ("%s is offering to send file %s"), buddy ? purple_buddy_get_alias (buddy) : xfer->who, purple_xfer_get_filename (xfer));
...
void purple_xfer_request_accepted (PurpleXfer *xfer, const char *filename) {
    ...
    if (type == PURPLE_XFER_SEND) {
        ...
        msg = g_strdup_printf (_ ("Offering to send %s to %s"), utf8, buddy ? purple_buddy_get_alias (buddy) : xfer->who);
        g_free (utf8);
        ...
        xfer->status = PURPLE_XFER_STATUS_ACCEPTED;
        purple_xfer_set_local_filename (xfer, filename);
        msg = g_strdup_printf (_ ("Starting transfer of %s from %s"), xfer->filename, buddy ? purple_buddy_get_alias (buddy) : xfer->who);
        purple_xfer_conversation_write (xfer, msg, FALSE);
        g_free (msg);
    }
...
void purple_xfer_set_completed (PurpleXfer *xfer, gboolean completed) {
    ...
    if (completed == TRUE) {
        ...
        if (purple_xfer_get_filename (xfer) != NULL) {
            ...
            if (purple_xfer_get_local_filename (xfer) && purple_xfer_get_type (xfer) == PURPLE_XFER_RECEIVE) {
                char *local = g_markup_escape_text (purple_xfer_get_local_filename (xfer), -1);
                msg = g_strdup_printf (_ ("Transfer of file <A HREF=\"file://%s\">%s</A> complete"), local, filename);
                g_free (local);
            }
            else
                msg = g_strdup_printf (_ ("Transfer of file %s complete"), filename);
            g_free (filename);
        }
        else
...
static void purple_xfer_increase_buffer_size (PurpleXfer *xfer) {
    xfer->current_buffer_size = MIN (xfer->current_buffer_size * 1.5, FT_MAX_BUFFER_SIZE);
}
...
gssize purple_xfer_write (PurpleXfer *xfer, const guchar *buffer, gsize size) {
    ...
    if (r >= 0 && (purple_xfer_get_bytes_sent (xfer) + r) >= purple_xfer_get_size (xfer) && !purple_xfer_is_completed (xfer))
...
static void begin_transfer (PurpleXfer *xfer, PurpleInputCondition cond) {
    ...
    if (ui_ops == NULL || (ui_ops->ui_read == NULL && ui_ops->ui_write == NULL)) {
        xfer->dest_fp = g_fopen (purple_xfer_get_local_filename (xfer), type == PURPLE_XFER_RECEIVE ? "wb" : "rb");
        if (xfer->dest_fp == NULL) {
...
void purple_xfer_start (PurpleXfer *xfer, int fd, const char *ip, unsigned  int port) {
    ...
    if (type == PURPLE_XFER_RECEIVE) {
        ...
        if (ip != NULL) {
            ...
            purple_proxy_connect (NULL, xfer -> account, xfer -> remote_ip, xfer -> remote_port, connect_cb, xfer);
            return;
        }
        else {
            xfer->fd = fd;
...
void purple_xfer_cancel_local (PurpleXfer *xfer) {
    ...
    xfer->end_time = time (NULL);
    if (purple_xfer_get_filename (xfer) != NULL) {
        msg = g_strdup_printf (_ ("You cancelled the transfer of %s"), purple_xfer_get_filename (xfer));
    }
    else {
...
void purple_xfer_cancel_remote (PurpleXfer *xfer) {
    ...
    buddy = purple_find_buddy (account, xfer->who);
    if (purple_xfer_get_filename (xfer) != NULL) {
        msg = g_strdup_printf (_ ("%s cancelled the transfer of %s"), buddy ? purple_buddy_get_alias (buddy) : xfer->who, purple_xfer_get_filename (xfer));
    }
    else {
        msg = g_strdup_printf (_ ("%s cancelled the file transfer"), buddy ? purple_buddy_get_alias (buddy) : xfer->who);
    }
    purple_xfer_conversation_write (xfer, msg, TRUE);
...
void purple_xfers_init (void) {
    ...
    xfers_data = g_hash_table_new_full (g_direct_hash, g_direct_equal, NULL, purple_xfer_priv_data_destroy);
    purple_signal_register (handle, "file-recv-accept", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-send-accept", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-recv-start", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-send-start", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-send-cancel", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-recv-cancel", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-send-complete", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-recv-complete", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
    purple_signal_register (handle, "file-recv-request", purple_marshal_VOID__POINTER, NULL, 1, purple_value_new (PURPLE_TYPE_SUBTYPE, PURPLE_SUBTYPE_XFER));
}

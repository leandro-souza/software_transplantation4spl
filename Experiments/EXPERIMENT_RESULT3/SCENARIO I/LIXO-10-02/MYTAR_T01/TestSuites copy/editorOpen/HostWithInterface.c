...
int main(int argc, char *argv[]) {
  ...
  if (argc >= 2) {
    //editorOpen(argv[1]);
    /*LOSCHANGE*/
    int editorOpen_TRANSPLANT_RESULT;
    editorOpen_TRANSPLANT_RESULT = editorOpen_GRAFT_INTERFACE (argc, argv,filename);
    /*LOSCHANGE*/
  }
...
int main (int argc, char *argv []) {
    ...
    if (argc >= 2) {
            int editorOpen_TRANSPLANT_RESULT;
        editorOpen_TRANSPLANT_RESULT = editorOpen_GRAFT_INTERFACE (argc, argv);
    }
    editorSetStatusMessage ("HELP: CTRL-S = save | Ctrl-Q = quit | Ctrl-F = find");
    while (1) {
        editorRefreshScreen ();
